...
- Colors are passed as a single **color value**: a named string (`"red"`, `"white"`, etc.), a hex string (`"#FF8800"`), or a table `{r = R, g = G, b = B}` where each field is an integer 0–255. Passing three separate integers for color is not supported.
