...
`claw_core` is the critical runtime path: it builds iteration context, calls the LLM backend, executes capabilities, persists context, handles interrupts, and emits responses. Changes under `framework/runtime/claw-core/` should be narrow and justified.
...
Lua drivers and modules live under `framework/lua_modules/` and are registered through `applications/shared/app_claw/app_lua_modules.c`. Hardware-specific modules should stay guarded by the existing Kconfig and board capability checks.
...
Shared build-time FATFS defaults live in `applications/edge_agent/fatfs_image/` with one subdirectory per partition. The SYSTEM source tree (`fatfs_image/system/`) is staged into `build/system_fs_image/`, then the selected board's `fatfs_image/` overlay is copied into that SYSTEM staging directory if present. Component skills and built-in Lua scripts/docs are then synced into the SYSTEM staging directory. Board overlay content does not target writable DATA storage, and hidden board folders are not considered.
