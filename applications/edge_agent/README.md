...
```text
applications/edge_agent/fatfs_image/
...
```text
applications/edge_agent/build/fatfs_image/        # storage staging
applications/edge_agent/build/system_fs_image/    # system staging
```
...
```text
applications/edge_agent/boards/<vendor>/<board>/fatfs_image/
```

During the build, `applications/edge_agent/CMakeLists.txt` first copies the base `fatfs_image/system/` directory into the system staging dir, then copies the selected board's `fatfs_image/` directory if it exists. The selected board path comes from the generated `components/gen_bmgr_codes/CMakeLists.txt`, which is produced by `idf.py bmgr`.
