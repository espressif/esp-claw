//! `subagent_list_spawnable()` — the menu of subagent kinds this agent may spawn.
//!
...
//! what it may spawn (and pick the right `kind` for `subagent_spawn`) instead of
//! guessing a kind and learning by rejection — and unlike baking the catalog into
//! `subagent_spawn`'s schema, it costs nothing in the always-sent prompt prefix.
