...
impl<F: ClawFs + 'static> LongTermMemory<F> {
    ...
    /// Number of live items. Mostly for tests/diagnostics.
...
mod tests {
    ...
    fn draft(content: &str, tags: &[&str]) -> MemoryDraft {
        MemoryDraft::new(content).with_tags(tags.iter().map(|t| t.to_string()))
    }
    ...
    fn store_dedups_by_normalized_content() {
        ...
        // Case- and whitespace-insensitive: a re-store is a no-op duplicate.
    ...
    fn recall_filters_by_label_and_query_newest_first() {
        ...
        // Empty labels matches everything; limit caps the result.
    ...
    fn state_survives_reload_from_journal() {
        ...
        // Next minted id continues past the highest replayed seq.
        assert_eq!(
            reloaded
