...
| `chrome_export` | Consumes a `claw_trace` forest and writes Chrome Trace Event JSON (`write_chrome_trace`). Spans → complete events, events → instant events, numeric event fields → counter tracks. Exposed as the `claw-trace-chrome` CLI. Depends on `chrometrace`. |
