...
`claw-tool`: tool framework — `Tool` / `ToolSet` traits, aggregation, permission gating, execution, and the build-time bake contract for embedding tool metadata assets at compile time.
...
`claw-capability`: capability adapter — maps registered `Capability` items into internal tool and channel roles plus an orthogonal enable/disable lifecycle. bridges the capability surface onto the orchestrator.
