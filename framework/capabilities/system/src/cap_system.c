...
static esp_err_t cap_system_execute_get_info_impl(const char *input_json,
                                                  char *output,
                                                  size_t output_size)
{
...
static esp_err_t cap_system_execute_get_current_time_impl(const char *input_json,
                                                          char *output,
                                                          size_t output_size)
{
    esp_err_t err;
...
static esp_err_t cap_system_execute_restart_impl(const char *input_json,
                                                 char *output,
                                                 size_t output_size)
{
    cJSON *input = NULL;
...
CLAW_CABI_ESP_TOOL_CALLBACK(cap_system_execute_get_info, cap_system_execute_get_info_impl)
CLAW_CABI_ESP_TOOL_CALLBACK(cap_system_execute_get_current_time, cap_system_execute_get_current_time_impl)
CLAW_CABI_ESP_TOOL_CALLBACK(cap_system_execute_restart, cap_system_execute_restart_impl)

static const claw_capability_t s_system_descriptors[] = {
    {
        .id = "get_system_info",
        ...
        .role_data.tool = {
            .schema_json = "{\"type\":\"object\",\"properties\":{\"sections\":{\"type\":\"array\",\"items\":{\"type\":\"string\","
    ...
    CLAW_CABI_ESP_TOOL_DESCRIPTOR(
        "get_current_time",
        "Return formatted current local time. Sync with SNTP only when the clock is invalid.",
        "{\"type\":\"object\",\"properties\":{}}",
        cap_system_execute_get_current_time),
    CLAW_CABI_ESP_TOOL_DESCRIPTOR(
        "restart_device",
        "Schedule a device restart after an optional delay in milliseconds.",
        "{\"type\":\"object\",\"properties\":{\"delay_ms\":{\"type\":\"integer\",\"minimum\":0}}}",
        cap_system_execute_restart),
};

static const claw_capability_group_t s_system_group = {
    .id = "cap_system",
    .members = s_system_descriptors,
    .member_count = sizeof(s_system_descriptors) / sizeof(s_system_descriptors[0]),
};
...
esp_err_t cap_system_register_group(claw_capability_registry_t *registry)
{
    ...
    esp_err_t err = claw_cabi_register_group_esp(registry, &s_system_group);
    if (err != ESP_OK) {
