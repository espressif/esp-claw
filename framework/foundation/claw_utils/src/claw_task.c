...
static const claw_task_config_t *claw_task_find_override(const char *name)
{
    ...
    for (size_t i = 0; s_task_configs[i].name; i++) {
...
static bool claw_task_resolve_config(const claw_task_config_t *input, claw_task_config_t *out)
{
                            ...
                            TaskHandle_t *task_handle)
{
    ...
    if (!claw_task_resolve_config(config, &resolved) || !task_func || resolved.stack_size == 0) {
    ...
    ESP_LOGD(TAG,
             "Creating task '%s' stack=%u priority=%u core=%d caps=%s",
             resolved.name,
