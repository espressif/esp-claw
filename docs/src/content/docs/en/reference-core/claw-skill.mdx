...
Header: <LinkToSource relativePath="framework/foundation/claw_skill/include/claw_skill.h" />
...
1. <LinkToSource relativePath="applications/edge_agent/fatfs_image/skills/" />: app-bundled factory Skills laid out as `skills/<skill_id>/SKILL.md`, copied into the build staging image.
2. **Component Skills**: each `cap_*` / `lua_module_*` / `lua_driver_*` component may ship `skills/<skill_id>/SKILL.md`; <LinkToSource relativePath="framework/build_support/skill_builder/" /> syncs them into `build/fatfs_image/skills/` before packaging.

Note: <LinkToSource relativePath="applications/edge_agent/fatfs_image/" /> is source input. `applications/edge_agent/build/fatfs_image/` is generated staging output and is recreated during the build.
