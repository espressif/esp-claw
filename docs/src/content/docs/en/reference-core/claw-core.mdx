...
Request/response clients start an explicitly independent run with `claw_core_start_run` and read `claw_core_response_t` via `claw_core_receive` / `claw_core_receive_for`.
`claw_core_response_t` includes assistant text (`text`) and `error_message`.

Conversation transports use `claw_core_post_message`. Core atomically decides whether the message belongs in the active run's inbox or starts a new queued run, and returns `claw_core_message_receipt_t` with the owning `run_id` and disposition. Callers must not read the current phase and then choose between separate submit and append APIs; that check would race with run finalization.
...
| `CLAW_CORE_REQUEST_FLAG_PUBLISH_STAGE_MESSAGE` | Publish Agent stage updates through the Event Router |

The Event Router's `run_agent` action sets these flags to enable asynchronous message delivery and event-published progress and responses.

## Messages and runs

A user message and an Agent run are different lifecycle objects. When `claw_core_post_message` receives a message for the session currently owned by an insertable run, Core:

1. copies the text into the run's bounded inbox;
2. aborts an in-flight LLM HTTP call cooperatively when necessary;
3. drains inbox messages at safe Agent-loop checkpoints;
4. persists each message as user context before rebuilding the next inference input.

The original run keeps ownership of the final response and cancellation. Messages received for another session, while the current run is finalizing, or while no run is active start a new queued run. Inbox capacity and queue submission are explicit backpressure boundaries rather than UI-level busy checks.
