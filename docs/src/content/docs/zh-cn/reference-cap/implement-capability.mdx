...
实现 Capability 前需理解两个核心结构体，均定义于 <LinkToSource relativePath="framework/foundation/claw_cap/include/claw_cap.h" />：
...
构建时 `framework/build_support/skill_builder` 会把这些文件同步到 FATFS 的 Skills 根目录（默认 `/fatfs/skills/`）下。
