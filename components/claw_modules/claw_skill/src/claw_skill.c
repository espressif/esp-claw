...
static esp_err_t claw_skill_request_skills_collect(
    const claw_core_request_t *request,
    claw_core_context_t *out_context,
    void *user_ctx)
{
    char *content = NULL;
    size_t content_capacity = 1U;
    size_t content_length = 0;
    esp_err_t err = ESP_OK;

    (void)user_ctx;
    if (!request || !out_context || !s_skill || !s_skill->initialized) {
        return ESP_ERR_INVALID_ARG;
    }
    memset(out_context, 0, sizeof(*out_context));
    ...
    for (size_t i = 0; i < request->skill_count; ++i) {
        const char *skill_id = request->skill_ids[i];
        size_t wrapper_bytes;

        if (!skill_id || !skill_id[0]) {
            return ESP_ERR_INVALID_ARG;
        }
        wrapper_bytes = 48U + strlen(skill_id);
        if (s_skill->max_file_bytes >
                SIZE_MAX - content_capacity - wrapper_bytes) {
            return ESP_ERR_INVALID_SIZE;
        }
        content_capacity += s_skill->max_file_bytes + wrapper_bytes;
    }

    content = calloc(1, content_capacity);
    if (!content) {
        return ESP_ERR_NO_MEM;
    }
    for (size_t i = 0; i < request->skill_count; ++i) {
        char *document = calloc(1, s_skill->max_file_bytes + 1U);
        int written;
        ...
        err = claw_skill_read_document(request->skill_ids[i], document,
                                       s_skill->max_file_bytes + 1U);
        if (err != ESP_OK) {
            free(document);
        ...
        written = snprintf(content + content_length,
                           content_capacity - content_length,
                           "\n<active_skill name=\"%s\">\n%s\n</active_skill>\n",
                           request->skill_ids[i], document);
        free(document);
        if (written < 0 ||
                (size_t)written >= content_capacity - content_length) {
            err = ESP_ERR_INVALID_SIZE;
            goto cleanup;
        }
...
const claw_core_context_provider_t claw_skill_request_skills_provider = {
    .name = "Request Skills",
    .collect = claw_skill_request_skills_collect,
    .user_ctx = NULL,
