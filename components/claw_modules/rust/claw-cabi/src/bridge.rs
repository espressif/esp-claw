//! Bridge: the populated capability [`Registry`] -> the agent runtime's seams.
//!
//! Pure Rust glue (no C ABI). The `Registry` owns capability identity and
//! lifecycle; the runtime consumes the *internal* representations it hands out:
//!
//! - [`RegistryResolver`] — a [`claw_core::agent::AgentResolver`] whose tools are
//!   the registry's currently-available [`Tool`]s (capability name -> `Tool`).
//!   Skills are an orthogonal concern owned by `claw-skill`; an optional
//!   [`SkillRegistry`] is threaded through unchanged.
//! - [`RegistryChannelTransport`] — wraps one channel [`ChannelAdapter`] as a
//!   [`claw_core::ChannelTransport`] for the [`ChannelEgressHub`], converting the
//!   (field-identical) outbound message types at the boundary so `claw-capability`
//!   keeps no upward dependency on `claw-core`.
//!
//! Inbound flows the other way through [`ClawCapabilityIngress`](crate::ClawCapabilityIngress):
//! the firmware's Rust entry point — which owns both the `Registry` and the
//! runtime — builds the `Orchestrator` (the [`ChannelIngressSink`](claw_core::ChannelIngressSink)),
//! installs this resolver, registers the channel transports via
//! [`register_channels`], and hands the orchestrator out as the ingress handle.
...
mod tests {
    ...
    use claw_capability::{Capability, CapabilityError, ChannelAdapter};
